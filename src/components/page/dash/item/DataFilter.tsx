...
export default function DataFilter({ list, property, data, children }) {
    ...
    return (
        <FilterContext.Provider value={{ list, filter, setFilter }}>
            <div className="h-full flex flex-col grow space-y-2">
                <List list={list}>
                    <Section />
                </List>
...
function Item() {
    ...
    return (
         <div className={clsx(isEnabled && "bg-white !text-black", "bg-gray-300 text-white px-3 rounded-full cursor-pointer select-none")} onClick={isEnabled ? remove : add}>{displayName}</div>
    )
}
